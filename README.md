...
Каждый может скопировать этот Git-репозиторий себе на компьютер и через несколько минут иметь собственную BIM-библиотеку на GitHub Pages. Ее можно использовать как отдельную библиотеку BIM-компонентов проектного бюро или производителя оборудования, личную библиотеку проектировщика, в т.ч. для хранения компонентов для других программных комплексов.

Этот раздел поможет вам начать работу с библиотекой BIM-компонентов *evoslib*. В нем описаны шаги по клонированию репозитория, установке Python и необходимых библиотек, а также запуску скриптов для подготовки данных.

В принципе, можно вообще обойтись без Python и без скриптов, но тогда данные в JSON придется добавлять вручную, а в папке `/components/` может начаться неконтролируемый хаос. 

#### 1. Клонирование репозитория

Для начала вам нужно склонировать репозиторий с GitHub на локальный компьютер. Для этого установите  выполните следующие шаги:

1. Установите Git: [https://git-scm.com/download/win/](https://git-scm.com/download/win/)
2. Откройте терминал (командную строку) на вашем компьютере (`Win+R` -> `cmd`).
3. Перейдите в директорию, куда вы хотите клонировать репозиторий (`cd c:\myfolder`).
4. Выполните команду:

    ```sh
    git clone https://nanoib.github.io/evoslib/
    ```

#### 2. Установка Python и необходимых библиотек к нему

Для работы со скриптами вам потребуется Python. Следуйте этим шагам для установки Python на Windows:

1. Скачайте: [https://www.python.org/downloads/](https://www.python.org/downloads/).
2. Установите, обязательно отметив опцию "Add Python to PATH" во время установки.
3. Перейдите в директорию проекта `pyscripts`:

    ```sh
    cd c:\myfolder\evoslib\pyscripts`
    ```

4. В проекте есть файл `requirements.txt`, который содержит список необходимых Python-библиотек. Для их установки выполните:

    ```sh
    pip install -r requirements.txt
    ```

После установки всех необходимых библиотек вы можете запустить скрипты для подготовки данных. Основной скрипт, который вам потребуется, это `xlsxtojson.py`. Он преобразует данные из Excel файла (`Base.xlsx`) в формат JSON (`Base.json`) и выполняет проверки целостности данных. 

Его можно запускать двойным кликом, а можно с помощью `python xlsxtojson.py` в терминале.

#### 3. Настройка собственного репозитория

1. Создайте аккаунт на GitHub, если у вас его еще нет. Используя New repository, создайте новый репозиторий для себя.

2. Перейдите в директорию проекта:

    ```sh
    cd c:\myfolder\evoslib
    ```

3.  Измените URL удаленного репозитория на свой собственный:

    ```sh
    git remote set-url origin https://github.com/<ваше_имя_пользователя>/<имя_вашего_репозитория>.git
    ```

#### 4. Отправка изменений в Git

Когда вы добавили, удалили или изменили BIM-компоненты, или, например, изменили код или readme-файл, тогда нужно отправить эти изменения в Git. Делайте так.

1. Добавьте измененные файлы в индекс Git и зафиксируйте эти изменения с сообщением. Да, сообщение обязательно. Так:
...
Теперь ваши изменения будут отправлены в удаленный репозиторий на GitHub. Чтобы заработал сайт на Github Pages, включите GitHub Pages в настройках GitHub. Подробнее об этом: 
[https://docs.github.com/ru/pages/getting-started-with-github-pages/creating-a-github-pages-site](https://docs.github.com/ru/pages/getting-started-with-github-pages/creating-a-github-pages-site)

Теперь процесс налажен.